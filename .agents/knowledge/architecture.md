...
### Đấu giá trực tuyến — engine trả giá (`20260913000001`)
- **Bảng:** `auction_lot_states` (1:1 lô, tạo lười, không có dòng = `pending`; realtime, REPLICA IDENTITY FULL) · `auction_bids` (chỉ ghi thêm, ngoại lệ `withdrawn_at`; realtime; `bidder_no` phi chuẩn hoá để công khai không JOIN hồ sơ) · `auction_lot_events` (nhật ký) · `auction_deposit_events` (sổ tiền đặt trước, ghi bằng TRIGGER trên `auction_bidding_contracts.deposit_status`) · `auction_session_minutes` (+ bucket private `auction-minutes`, path `{org}/{session}/{tên}.pdf`). Cột mới trên `auction_sessions`: `bidding_method`, `extension_seconds`, `max_bid_steps`, `finalized_at/by`.
- **RLS:** trạng thái lô + lượt trả giá công khai theo `auction_session_is_public`; nhật ký chỉ tổ chức; sổ tiền đặt trước = tổ chức + chính người tham gia (`auction_deposit_event_visible`); biên bản = tổ chức, công khai khi `finalized_at` (object storage chỉ khi đã phát hành thành biên bản). Không policy ghi; REVOKE INSERT/UPDATE/DELETE/TRUNCATE khỏi anon/authenticated.
- **Ghi — chỉ qua RPC** trả `{ok, reason}` (dùng `assertRpcOk`): `place_bid(_lot_id,_amount,_nonce)`, `withdraw_bid`, `org_open_lot`, `org_pause_lot`, `org_resume_lot`, `org_withdraw_lot`, `org_finalize_session`, `org_issue_minutes`, `org_confirm_winner_payment`, `org_mark_deposit_refunded`; `server_now()` cho lệch đồng hồ. Quyền qua `can_run_auction(org, action)` / `can_run_auction_session`.
- **Đồng thời:** khoá tư vấn `'lot:'||lot_id` rồi `SELECT … FOR UPDATE`; thời gian `clock_timestamp()` lấy SAU khi khoá. Nonce UNIQUE theo (hồ sơ, nonce) ⇒ gửi lại trả đúng lượt cũ. Chốt phiên khoá `'bidding:'||session_id` (cùng khoá với RPC hồ sơ tham gia) + mọi lô theo thứ tự id.
- **Đóng lô:** pg_cron `close_due_lots` mỗi 10 giây (REVOKE khỏi mọi vai) + đóng lười trong mọi RPC chạm lô quá giờ (`_close_lot` kiểm lại `ends_at` sau khi khoá). Job `purge_cron_history` dọn `cron.job_run_details` > 3 ngày.
- **Guard:** trigger khoá cấu hình phiên / lô / tiền đặt trước khi đã bắt đầu trả giá; RPC lách bằng `_bidding_ctx()` (GUC cục bộ `app.bidding_rpc`, kèm `app.deposit_reason` / `app.deposit_lot_id` cho sổ tiền đặt trước) và xoá bằng `_bidding_ctx_clear()`.
- **Client (Bước 3):** `src/lib/bidding/{rules,lotPhase,errors,countdown,roomAccess}.ts` + hook `useLotStates` (kênh realtime DUY NHẤT của phiên, nghe cả `auction_lot_states` lẫn `auction_bids`) / `useLotBids` / `usePlaceBid` / `useServerClock` / `useMyBidderStatus` / `useOrgBidding`. Query key `qk.bidding.*`. `assertBiddingRpcOk` riêng, KHÔNG dùng `assertRpcOk` của ký gửi.
- **Phòng trả giá (Bước 4):** `/sessions/:id/dau-gia` → `AuctionBiddingRoomPage` (vỏ + cổng `roomGateOf`) → `src/components/bidding-room/BiddingRoom.tsx` (nơi DUY NHẤT mount `useLotStates`; tách khỏi trang để không mở socket cho người bị chặn) + `LotBoard` / `LiveLotCard` / `BidPanel` / `BidHistory` / `WithdrawBidDialog` / `EligibilityGate` / `LotPhaseBadge` / `BiddingMethodNotice`. Chọn lô qua `?lot=<lot_no>`. Lối vào: `SessionBiddingEntryCard` trên `/sessions/:id`. Bảng tin công khai dựng từ `auction_bids` + `auction_lot_states`, KHÔNG từ `auction_lot_events` (anon không đọc được).

### Hợp đồng mua bán tài sản đấu giá — sau đấu giá (`20260914000001-2`)
- **Bảng:** `auction_sale_contracts` (MỘT hợp đồng cho MỘT lô đã bán, mã `HDMB%06d`; partial unique `uq_asc_one_open_per_lot` chỉ chặn hợp đồng `status <> 'cancelled'` nên huỷ rồi lập lại được) · `auction_sale_installments` (lịch kỳ hạn) · `auction_sale_payments` (SỔ CHỈ GHI THÊM; hoàn = DÒNG MỚI có `reversed_payment_id`, không bao giờ UPDATE) · `auction_sale_contract_events` (nhật ký). Bucket private `auction-sale-contracts`, path `{org}/{contract}/{draft|signed|receipt|handover|title}-{epoch}-{tên}`.
- **Bên bán suy từ NGUỒN của lô** (`sale_seller_snapshot`): lô `posting` → chủ tài sản có tài khoản (`consignment_contracts.owner_user_id`) ⇒ `seller_kind='owner_user'`, tự thao tác trong cổng chủ tài sản; lô `listing` → thực thể danh bạ `asset_owners` KHÔNG có tài khoản ⇒ `seller_kind='org_on_behalf'`, tổ chức ký thay. Không giải được ⇒ `seller_unresolved` (hợp đồng ký gửi đã huỷ, hoặc `listings.asset_owner_id` NULL — ~20% tin đăng).
- **RLS chỉ ĐỌC, không policy công khai:** bên mua (`buyer_user_id`), bên bán (`seller_user_id`), thành viên tổ chức có `hop-dong-mua-ban.view`, admin. Ba bảng con qua `sale_contract_visible(contract_id)`. Dòng hợp đồng mang CCCD CẢ HAI bên — không bao giờ lộ cho anon (mục kiểm chứng của migration khẳng định bucket không có policy anon).
- **Ghi — chỉ qua RPC** trả `{ok, reason}` (dùng `assertSaleRpcOk` RIÊNG): `org_create_sale_contract`, `sale_contract_set_terms`, `sale_contract_share_draft`, `sale_contract_attach_signed`, `sale_contract_confirm`, `sale_contract_cancel`, `org_record_sale_payment`, `org_reverse_sale_payment`, `sale_contract_schedule_handover`, `sale_contract_confirm_handover`, `sale_contract_set_title_transfer`, `sale_contract_detail` (một vòng gọi cho cả trang), `org_sale_contract_counts`, `owner_sale_contract_summary`.
- **Tiền:** số dư = `price − deposit_credit − Σ(thu ròng)`; phân bổ FIFO TÍNH LẠI TỪ ĐẦU (`_sale_reallocate`) nên một bút toán hoàn tự mở lại đúng các kỳ đã đóng. Khi số dư về 0, `_sale_settle` đặt `paid_at` VÀ lật `auction_lot_states.payment_status='paid'` — đúng cột Bước 6 đọc. Bản song sinh thuần: `src/lib/saleContracts/money.ts`.
