import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";
import { useEffect, useRef, useState } from "react";
import { Gift, ArrowRight, Coins, CheckCircle2, Sparkles } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { useOnboardingTasks } from "@/hooks/useOnboardingTasks";
import { RewardClaimDialog } from "@/components/onboarding/RewardClaimDialog";
import { cn } from "@/lib/utils";

interface Props {
  open: boolean;
  onOpenChange: (o: boolean) => void;
}

export const RewardTasksDialog = ({ open, onOpenChange }: Props) => {
  const navigate = useNavigate();
  const { tasks, availableCredits, refresh } = useOnboardingTasks();
  const [claimDialogKey, setClaimDialogKey] = useState<"basic" | "intent" | null>(null);
  const autoClaimedRef = useRef<Set<string>>(new Set());

  // Refresh tasks each time the dialog opens to ensure freshness
  useEffect(() => {
    if (open) {
      refresh();
    } else {
      autoClaimedRef.current.clear();
    }
  }, [open, refresh]);

  // Auto-claim any ready task by opening the celebration dialog (which performs the claim)
  useEffect(() => {
    if (!open || claimDialogKey) return;
    const ready = tasks.find(
      (t) => t.status === "ready" && !autoClaimedRef.current.has(t.key)
    );
    if (ready) {
      autoClaimedRef.current.add(ready.key);
      setClaimDialogKey(ready.key);
    }
  }, [open, tasks, claimDialogKey]);

  const handleGo = (anchor: string) => {
    onOpenChange(false);
    navigate(anchor);
  };

  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <div className="mx-auto h-14 w-14 rounded-full bg-gradient-to-br from-primary/30 to-primary/10 flex items-center justify-center mb-2">
              <Gift className="h-7 w-7 text-primary" />
            </div>
            <DialogTitle className="text-center">Nhiệm vụ nhận thưởng</DialogTitle>
            <DialogDescription className="text-center">
              {availableCredits > 0
                ? `Đang cộng ${availableCredits} credit vào tài khoản…`
                : "Hoàn thành để nhận credit miễn phí"}
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-3 mt-2">
            {tasks.map((task) => {
              const isClaimed = task.status === "claimed";
              const isReady = task.status === "ready";

              return (
                <div
                  key={task.key}
                  className={cn(
                    "rounded-xl border p-4 transition-all",
                    isClaimed
                      ? "border-border bg-muted/40 opacity-60"
                      : isReady
                        ? "border-primary/50 bg-primary/5 shadow-sm ring-1 ring-primary/20"
                        : "border-primary/30 bg-card hover:border-primary/50"
                  )}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-2 min-w-0 flex-1">
                      {isClaimed ? (
                        <CheckCircle2 className="h-5 w-5 text-primary shrink-0 mt-0.5" />
                      ) : isReady ? (
                        <Sparkles className="h-5 w-5 text-primary shrink-0 mt-0.5" />
                      ) : (
                        <div className="h-5 w-5 rounded-full border-2 border-primary/40 shrink-0 mt-0.5" />
                      )}
                      <div className="min-w-0">
                        <p className={cn(
                          "font-semibold text-sm",
                          isClaimed ? "text-muted-foreground line-through" : "text-foreground"
                        )}>
                          {task.title}
                        </p>
                        <p className="text-xs text-muted-foreground mt-0.5">{task.description}</p>
                      </div>
                    </div>
                    <div className="shrink-0">
                      {isClaimed ? (
                        <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-xs font-medium text-muted-foreground">
                          <CheckCircle2 className="h-3 w-3" />Đã nhận
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 text-xs font-semibold text-primary">
                          <Coins className="h-3 w-3" />+{task.credits}
                        </span>
                      )}
                    </div>
                  </div>

                  {!isClaimed && !isReady ? (
                    <Button
                      onClick={() => handleGo(task.anchor)}
                      variant="outline"
                      className="w-full mt-3"
                      size="sm"
                    >
                      Đi đến {task.key === "basic" ? "hồ sơ" : "khai báo nhu cầu"}
                      <ArrowRight className="h-4 w-4" />
                    </Button>
                  ) : null}
                </div>
              );
            })}
          </div>
        </DialogContent>
      </Dialog>

      {claimDialogKey && (
        <RewardClaimDialog
          open={Boolean(claimDialogKey)}
          onOpenChange={(o) => {
            if (!o) {
              setClaimDialogKey(null);
              refresh();
            }
          }}
          taskKey={claimDialogKey}
        />
      )}
    </>
  );
};
